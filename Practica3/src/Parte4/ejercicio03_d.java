...
public class ejercicio03_d {
	    ...
	    if (!catalogo.esvaciocatalogo())
	    {
	        ...
	        do {
		        ...
		        switch (opcion)
		        {
		            ...
		            case "3": System.out.println(catalogoinsumo.ListatoCostos(catalogo));break;
	...
	else System.out.println("El catalogo esta vacio");
	}
    ...
    public static void cargarcatalogocosto()
	{
	    ...
	    {
	        Insumo aux = new Insumo(nodo.getCodigo(), null);
	        catalogoinsumo.insertarcatalogoinsumo(aux);
	    }
	...
	public static void AltaCosto() throws IOException
	{
	    ...
	    {
	        nodo = catalogo.BuscarConcepto(codigo);
	        costo = Libreria.dialogo("Introduce el costo del Insumo " + nodo.getInsumo());
	        Insumo auxnodo = new Insumo(nodo.getCodigo(), costo);
	        catalogoinsumo.actualizarcatalogoinsumo(auxnodo);
	    }
	    else
	...
	public static void ModificarCosto() throws IOException
	{
	    ...
	    if (catalogo.existe(codigo))
	    {
	        costo = catalogoinsumo.checarcostoinsumo(codigo);
	        nodo = catalogo.BuscarConcepto(codigo);
	        String cadena = "";
	        if (costo != null)
	            cadena = "El precio actual es " + costo + " Introduce el costo del Insumo " + nodo.getInsumo();
	        else
	            cadena = "Introduce el costo del Insumo " + nodo.getInsumo();
	        costo = Libreria.dialogo(cadena);
	        Insumo aux = new Insumo(codigo, costo);
	        catalogoinsumo.actualizarcatalogoinsumo(aux);
	    }
	    else
	...
	public static void AltasConceptos(int segmento, String ssegmento) throws IOException
	{
	    ...
	    nodo.setCodigo(Libreria.dialogo("Introduce el codigo del Concepto "));
	    if( !catalogo.existe(nodo.getCodigo().trim()) )
	    {
	        nodo.setInsumo(Libreria.dialogo("Introduce el nombre del Concepto "));
	        nodo.setDescripcion(Libreria.dialogo("Introduce la descripcion del Concepto "));
	        nodo.setUnidadmedida(Libreria.dialogo("Introduce la unidad del Concepto "));
	        catalogo.insertarinsumo( segmento ,nodo);
	    }
	    else
	...
	public static void Listado() throws IOException
	{
	    ...
	    do {
	        ...
	        switch (opcion)
	        {
	            case "1": System.out.println(catalogo.MostrarCatalogo(1));break;
	            case "2": System.out.println(catalogo.MostrarCatalogo(2));break;
	            case "3": System.out.println(catalogo.MostrarCatalogo(3));break;
	            case "4": System.out.println(catalogo.MostrarCatalogo(4));break;
	            case "5": System.out.println(catalogo.MostrarCatalogo(0));break;
	            case "6": System.out.println("elegiste salida");break;
	            default: System.out.println("Esta opcion no existe");break;
	        }
	    } while ((opcion.compareTo("6")!=0) && (!opcion.isEmpty()));
	...
	public static void BajasConceptos() throws IOException
	{
	    ...
	    {
	        nodo = catalogo.BuscarConcepto(codigo);
	        opcion = Libreria.dialogo("Deseas elimina a " + nodo.getInsumo() + " [Y/N]").toUpperCase().trim();
	        if (opcion.compareTo("Y") == 0)
	        {
	            catalogo.RemoverConceptos(codigo);
	        }
	    }
	    else
	        System.out.println("el codigo no existe no se puede eliminar");
